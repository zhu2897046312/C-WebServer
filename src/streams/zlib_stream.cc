...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    FANSHUTOU_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    FANSHUTOU_ASSERT((window_bits >= 8 && window_bits <= 15));
    FANSHUTOU_ASSERT((memlevel >= 1 && memlevel <= 9));
...
fst::ByteArray::ptr ZlibStream::getByteArray() {
